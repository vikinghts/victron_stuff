...
class DbusMqttService:
  def __init__(self, servicename, paths, productname='Mqtt 1PM', connection='Mqtt 1PM HTTP JSON service'):
  ...
  def _getMqttFWVersion(self):
    ...
    if not meter_data['sys']['available_updates']['stable']['version']:
        raise ValueError("Response does not contain 'update/old_version' attribute")

    ver = meter_data['sys']['available_updates']['stable']['version']
    return ver
  ...
  def _getMqttData(self):
    ...
    # check for response
    if not meter_r:
        raise ConnectionError("No response from Mqtt 1PM - %s" % (URL))
